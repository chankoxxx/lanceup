...
export function ThanksContent() {
  ...
  return <>
    ...
    {confirmation.contactMethod === "line" ? <div className="contact-next line-next"><span className="contact-next-icon" aria-hidden="true">LINE</span><div><h2>LINEで相談を続ける</h2><p>{canPrefill ? "ボタンを押すと、問い合わせ番号とお名前が入力された状態でLINEが開きます。内容を確認して送信してください。" : "公式アカウントを開き、上のお問い合わせ番号をメッセージでお送りください。"}</p>{lineUrl ? <a className="btn line-button" href={lineUrl} target="_blank" rel="noreferrer">{canPrefill ? "LINEを開く" : "LINE公式アカウントを開く"} <span aria-hidden="true">→</span></a> : <p className="setup-notice">LINE公式アカウントとの接続先を準備中です。設定完了後、このボタンから続けられます。</p>}</div></div> : <div className="contact-next email-next"><span className="contact-next-icon" aria-hidden="true">✉</span><div><h2>メールでご連絡します</h2><p>ご入力いただいたメールアドレスへ、内容を確認後に担当者からご連絡します。</p></div></div>}
